   ...
   fDetectorConstruction(Det)
{
  ...
  fEfficiencyCmd = new G4UIcmdWithADouble("/G/det/efficiency",this);
  fEfficiencyCmd->SetGuidance("Set the efficiency of the surface .");
  fEfficiencyCmd->SetParameterName("efficiency",false);
  fEfficiencyCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
  fEfficiencyCmd->SetToBeBroadcasted(false);
...
void G1DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
          ...
          if( command == fEfficiencyCmd ) 
	  {
	    fDetectorConstruction->SetEfficiency( fEfficiencyCmd->GetNewDoubleValue(newValue) );
	  }  
