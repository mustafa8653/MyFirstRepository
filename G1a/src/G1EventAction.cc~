...
: G4UserEventAction(), fRunAction(runAction), fHistoManager(histo), fEdep(0.), fIndex(0), fPmtPhotonCount(0)
{
...
void G1EventAction::EndOfEventAction (const G4Event* anEvent)
{
   ...
   //eventInformation->PrintEventResult();
    ...
    fPmtPhotonCount = eventInformation->GetPmtPhotonCount();
    
    if(fEdep == 0 && fPmtPhotonCount == 0 ) return;
    std::vector< double> pev = *(eventInformation->GetPhotonEnergyVec());
    
    
    
    
    fHistoManager->FillNtuple1(fEdep, fPmtPhotonCount);
    fHistoManager->FillNtuple3(pev);
    
    
    fRunAction->fillPerEvent(fEdep, fIndex);
    
    fHistoManager->FillHisto(0, fEdep);
    fHistoManager->FillHisto(2,fPmtPhotonCount);
       ...
       G4int weight = (*ii).second;
        ...
        //fHistoManager->FillHisto (1, index, weight);
          ...
          for(G4int i=0; i< weight; i++)
          {
              ...
              fHistoManager->FillNtuple2(index);     
              fHistoManager->FillHisto(1,index);   
